...
class ParserTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        r.add_field('end_month', transform_ops='int', check_ops=month_check, ignore_if=ignore_months)
        r.add_field('percentage', transform_ops=int, check_ops=perc_check)
        r.add_field('state', transform_ops=[lower, comma_split, state_filter], check_ops=len)
